...
<head>
  ...
  <title>Cookie Bridge — Visual</title>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#07070f;color:#ccc;font-family:'Segoe UI',Arial,sans-serif;min-height:100vh;padding-bottom:32px}
    /* Header */
    #hdr{background:linear-gradient(135deg,#080d1a,#0d1830);border-bottom:2px solid #b8872a;padding:12px 24px;display:flex;align-items:center;gap:18px;flex-wrap:wrap}
    #hdr-ck{width:42px;height:42px;animation:spin 20s linear infinite;filter:drop-shadow(0 0 8px #b8872a);flex-shrink:0}
    @keyframes spin{to{transform:rotate(360deg)}}
    #bakery{color:#f5e642;font-size:19px;font-weight:700;letter-spacing:0.5px;flex:1;min-width:120px}
    .hst{display:flex;flex-direction:column;min-width:70px}
    .hst-l{color:#1a2a40;font-size:8px;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:2px}
    .hst-v{color:#ccc;font-size:14px;font-weight:600}
    .hdr-btn{background:#0a1525;color:#5bc8f5;border:1px solid #1a3050;padding:4px 14px;border-radius:20px;font-size:11px;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center}
    /* Alert bar */
    #alerts{display:flex;gap:7px;padding:8px 24px;background:#050510;border-bottom:1px solid #0a0a18;flex-wrap:wrap}
    .alrt{display:flex;align-items:center;gap:5px;background:#09090f;border:1px solid #0d0d1e;border-radius:6px;padding:5px 10px}
    .alrt.on{border-color:#2d9e54;background:#091209}
    .alrt.shimmer{border-color:#ffd700;background:#130f00;animation:gpulse 1s ease-in-out infinite}
    @keyframes gpulse{0%,100%{box-shadow:0 0 6px #ffd70088}50%{box-shadow:0 0 20px #ffd700}}
    .alrt-l{color:#1a2a40;font-size:8px;letter-spacing:1px;text-transform:uppercase;white-space:nowrap}
    .alrt-v{color:#556;font-size:11px}
    .alrt.on .alrt-v{color:#2d9e54}
    .alrt.shimmer .alrt-v{color:#ffd700}
    .gcimg{animation:gcspn 3s linear infinite}
    @keyframes gcspn{to{transform:rotate(360deg)}}
    /* Buildings */
    #bsec{padding:14px 24px}
    .sec-t{color:#1a2a40;font-size:9px;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #0a0a18}
    #bgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(255px,1fr));gap:4px}
    .br{display:flex;align-items:center;gap:8px;background:#09090f;border:1px solid #0d0d1e;border-radius:5px;padding:5px 8px;transition:border-color .2s}
    .br:hover{border-color:#1a3050}
    .bi{width:32px;height:32px;background-image:url('/img/buildings.png');background-size:auto 32px;background-repeat:no-repeat;flex-shrink:0;image-rendering:pixelated;image-rendering:crisp-edges}
    .bn{color:#3a5060;font-size:10px;width:125px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .ba{color:#f5e642;font-size:13px;font-weight:700;width:42px;text-align:right;flex-shrink:0}
    .bb-w{flex:1;background:#0a1020;border-radius:2px;height:3px}
    .bb{height:3px;border-radius:2px;background:linear-gradient(90deg,#1a4d8f,#5bc8f5);transition:width .8s}
    /* Minigames */
    #msec{padding:4px 24px 14px}
    #mgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}
    .mc{background:#09090f;border:1px solid #0d0d1e;border-radius:8px;padding:10px 12px}
    .mc-t{color:#1a2a40;font-size:8px;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px}
    .mc-v{color:#aaa;font-size:11px;line-height:1.4}
    .mbar-w{background:#0a1020;border-radius:2px;height:4px;margin-top:6px}
    .mbar{height:4px;border-radius:2px;transition:width .5s,background .5s}
    /* Charts */
    #csec{padding:14px 24px 20px}
    .cw{background:#09090f;border:1px solid #0d0d1e;border-radius:8px;padding:14px 16px;margin-bottom:8px}
    /* Status bar */
    #sbar{position:fixed;bottom:0;left:0;right:0;background:#050510;border-top:1px solid #0a0a18;padding:4px 24px;display:flex;align-items:center;gap:12px;font-size:10px;color:#1a2a40;z-index:100}
    #sbar-st{color:#2d9e54}
  </style>
</head>
<body>
...
<!-- HEADER -->
<div id="hdr">
  <img id="hdr-ck" src="/img/cookie.png" onerror="this.style.display='none'" alt="">
  <div id="bakery">conectando...</div>
  <div class="hst"><span class="hst-l">Cookies</span><span class="hst-v" id="h-ck">—</span></div>
  <div class="hst"><span class="hst-l">CpS</span><span class="hst-v" id="h-cps">—</span></div>
  <div class="hst"><span class="hst-l">CpC</span><span class="hst-v" id="h-cpc">—</span></div>
  <div class="hst"><span class="hst-l">Prestige</span><span class="hst-v" id="h-prest">—</span></div>
  <div class="hst"><span class="hst-l">Ascensoes</span><span class="hst-v" id="h-asc">—</span></div>
  <a href="/docs" class="hdr-btn">&#x2190; Docs</a>
</div>

<!-- ALERT BAR -->
<div id="alerts">
  <div class="alrt" id="al-buf"><span class="alrt-l">Buffs</span><span class="alrt-v" id="al-buf-v">nenhum</span></div>
  <div class="alrt" id="al-sh"><span id="al-gc"></span><span class="alrt-l">Shimmers</span><span class="alrt-v" id="al-sh-v">nenhum</span></div>
  <div class="alrt" id="al-wr"><span class="alrt-l">Wrinklers</span><span class="alrt-v" id="al-wr-v">nenhum</span></div>
  <div class="alrt" id="al-lp"><span class="alrt-l">Lumps</span><span class="alrt-v" id="al-lp-v">0</span></div>
  <div class="alrt" id="al-sn"><span class="alrt-l">Estacao</span><span class="alrt-v" id="al-sn-v">—</span></div>
</div>

<!-- BUILDINGS -->
<div id="bsec">
  <div class="sec-t">Edificios</div>
  <div id="bgrid"><div style="color:#1a2a40;font-size:11px;padding:8px">Aguardando estado do jogo...</div></div>
</div>

<!-- MINIGAMES -->
<div id="msec">
  <div class="sec-t">Minijogos</div>
  <div id="mgrid">
    <div class="mc"><div class="mc-t">Grimorio</div><div class="mc-v" id="mg-gr">—</div><div class="mbar-w"><div class="mbar" id="mg-gr-b" style="width:0%"></div></div></div>
    <div class="mc"><div class="mc-t">Panteao</div><div class="mc-v" id="mg-pt">—</div></div>
    <div class="mc"><div class="mc-t">Jardim</div><div class="mc-v" id="mg-gd">—</div></div>
    <div class="mc"><div class="mc-t">Bolsa</div><div class="mc-v" id="mg-bk">—</div></div>
  </div>
</div>

<!-- CHARTS -->
<div id="csec">
  <div class="sec-t">Curva de Evolucao</div>
  <div class="cw"><canvas id="ch-cps" height="120"></canvas></div>
  <div class="cw"><canvas id="ch-prest" height="120"></canvas></div>
</div>

<!-- STATUS BAR -->
<div id="sbar">
  <span>Cookie Bridge Visual &mdash; atualiza a cada 2s</span>
  <span id="sbar-st">conectando...</span>
  <span style="flex:1"></span>
  <span id="sbar-ts"></span>
</div>

<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
(function() {
  // Sprite sheet column by building ID 0-19
  // buildings.png: 48x48px each, scaled to 32x32 via CSS
  // Historical order differs from current game order (Bank/Temple/Wizard added later)
  var BSPR = [0,1,2,3,4,15,16,17,5,6,7,8,13,14,19,20,32,33,34,35];

  var _FMT_NS=['million','billion','trillion','quadrillion','quintillion','sextillion','septillion','octillion','nonillion','decillion','undecillion','duodecillion','tredecillion','quattuordecillion','quindecillion','sexdecillion','septendecillion','octodecillion','novemdecillion','vigintillion'];
  function fmt(n) {
    ...
    var s;
    if (n >= 1e6) {
      var e = Math.min(Math.floor(Math.log10(n)/3)*3, 63);
      var idx = e/3 - 2;
      s = (idx >= 0 && idx < _FMT_NS.length) ? (n/Math.pow(10,e)).toFixed(3)+' '+_FMT_NS[idx] : n.toExponential(3);
    } else if (n >= 1000) {
  ...
  function setSt(id, k, v) { var e = document.getElementById(id); if (e) e.style[k] = v; }
  ...
  function update() {
    fetch('/state').then(function(r) { return r.json(); }).then(function(d) {
      ...
      var buffs = Object.keys(d.buffs_ativos || {});
      var sh = d.shimmers || [];
      var wr = (d.wrinklers || []).filter(function(w) { return w && w.phase > 0; });
      var cps = d.cookies_por_segundo_raw || d.cookies_por_segundo || 0;

      // Header
      setTxt('bakery', d.bakery_name || '—');
      setTxt('h-ck',   fmt(d.cookies_na_conta));
      setTxt('h-cps',  fmt(cps));
      setTxt('h-cpc',  fmt(d.cookies_por_click));
      var p = es.prestige || 0;
      setTxt('h-prest', p > 1e9 ? (p/1e9).toFixed(3)+'B' : p > 1e6 ? (p/1e6).toFixed(2)+'M' : String(p));
      setTxt('h-asc', '#' + (es.ascensoes || 0));

      // Buffs
      var bv = buffs.length ? buffs.slice(0,3).join(' / ')+(buffs.length>3?' +'+(buffs.length-3):'') : 'nenhum';
      setTxt('al-buf-v', bv);
      setCls('al-buf', 'alrt'+(buffs.length ? ' on' : ''));

      // Shimmers
      var gcEl = document.getElementById('al-gc');
      if (sh.length) {
        var red = sh.some(function(s) { return s.type === 'red'; });
        gcEl.innerHTML = '<img class="gcimg" src="/img/'+(red?'wrathCookie':'goldCookie')+'.png" style="width:26px;height:26px;margin-right:5px" onerror="this.style.display=\'none\'">';
        setCls('al-sh', 'alrt shimmer');
        setTxt('al-sh-v', sh.length+' na tela!');
      } else {
        gcEl.innerHTML = '';
        setCls('al-sh', 'alrt');
        setTxt('al-sh-v', 'nenhum');
      }

      // Wrinklers
      setTxt('al-wr-v', wr.length ? wr.length+' ativos' : 'nenhum');
      setCls('al-wr', 'alrt'+(wr.length ? ' on' : ''));

      // Lumps
      var lp = sl.disponiveis || 0;
      setTxt('al-lp-v', lp > 0 ? lp+' disponivel' : '0');
      setCls('al-lp', 'alrt'+(lp > 0 ? ' on' : ''));

      // Season
      var sn = es.estacao || (d.estatisticas && d.estatisticas.estacao) || 'none';
      setTxt('al-sn-v', sn && sn !== 'none' ? sn : '—');
      setCls('al-sn', 'alrt'+(sn && sn !== 'none' ? ' on' : ''));

      // Buildings
      var bldgs = d.buildings || [];
      if (bldgs.length) {
        maxAmt = Math.max.apply(null, bldgs.map(function(b) { return b.amount || 0; })) || 1;
        if (!bldgInit) {
          bldgInit = true;
          var grid = document.getElementById('bgrid');
          grid.innerHTML = '';
          bldgs.forEach(function(b) {
            var col = (b.id >= 0 && b.id < BSPR.length) ? BSPR[b.id] : 0;
            var bgX = -(col * 32);
            var pct = Math.round((b.amount / maxAmt) * 100);
            var div = document.createElement('div');
            div.className = 'br';
            div.innerHTML =
              '<div class="bi" style="background-position:'+bgX+'px 0px" title="'+b.name+'"></div>'+
              '<div class="bn">'+b.name+'</div>'+
              '<div class="ba" id="ba'+b.id+'">'+b.amount+'</div>'+
              '<div class="bb-w"><div class="bb" id="bb'+b.id+'" style="width:'+pct+'%"></div></div>';
            grid.appendChild(div);
          });
        } else {
          bldgs.forEach(function(b) {
            setTxt('ba'+b.id, b.amount);
            setW('bb'+b.id, Math.round((b.amount / maxAmt) * 100));
          });
        }
      }
      ...
      // Minigames — Grimoire
      var mn = gr.magic || 0;
      var mx = gr.magicMax || 100;
      var mpct = Math.min(100, Math.round((mn / mx) * 100)) || 0;
      setTxt('mg-gr', Math.round(mn)+'/'+mx+' mana');
      setW('mg-gr-b', mpct);
      setSt('mg-gr-b', 'background', mpct > 80 ? '#5bc8f5' : mpct > 40 ? '#2d9e54' : '#a05a00');

      // Minigames — Pantheon
      setTxt('mg-pt', (pt.swapsDisponiveis !== undefined ? pt.swapsDisponiveis : '?')+' swaps');

      // Minigames — Garden
      setTxt('mg-gd', gd.available ? 'ativo '+(gd.width||'?')+'x'+(gd.height||'?') : 'offline');

      // Minigames — Stock
      setTxt('mg-bk', bk.available ? 'negociacao ativa' : 'offline');

      // Status bar
      setSt('sbar-st', 'color', '#2d9e54');
      setTxt('sbar-st', 'conectado');
      setTxt('sbar-ts', new Date().toLocaleTimeString());

    }).catch(function() {
      setSt('sbar-st', 'color', '#ff6b6b');
      setTxt('sbar-st', 'desconectado');
      setTxt('bakery', 'DESCONECTADO - Cookie Clicker ativo?');
    });
  }

  // Charts from backup history — load once
  var chartsLoaded = false;
  function loadCharts() {
    if (chartsLoaded) return;
    fetch('/backup/history?n=500').then(function(r) { return r.json(); }).then(function(rows) {
      if (!rows || !rows.length) {
        document.getElementById('csec').innerHTML =
          '<div class="sec-t">Curva de Evolucao</div>'+
          '<div style="color:#1a2a40;font-size:11px;padding:16px 0">'+
          'Sem dados de backup ainda. O sistema coleta registros a cada 10 min.<br>'+
          'Arquivo: C:\\Users\\PICHAU\\CookieBridge\\history.ndjson</div>';
        return;
      }
      chartsLoaded = true;
      var labels = rows.map(function(r) {
        return r.ts ? r.ts.slice(5,16).replace('T',' ') : '';
      });
      var chOpts = {
        responsive: true,
        ...
        animation: false,
        plugins: {
          legend: { labels: { color:'#3a5060', font:{size:10} } }
        },
        scales: {
          x: { ticks:{color:'#1a2a40',maxTicksLimit:10,font:{size:8}}, grid:{color:'#0a0a14'} },
          y: { ticks:{color:'#1a2a40',font:{size:9}},                  grid:{color:'#0a0a14'} }
        }
      };
      new Chart(document.getElementById('ch-cps'), {
        type: 'line',
        data: {
          labels: labels,
          datasets: [{
            label: 'CpS',
            data: rows.map(function(r) { return r.cps || 0; }),
            borderColor: '#2d9e54',
            backgroundColor: 'rgba(45,158,84,0.07)',
            borderWidth: 1.5, pointRadius: 0, fill: true, tension: 0.3
          }]
        },
        options: chOpts
      });
      new Chart(document.getElementById('ch-prest'), {
        type: 'line',
        data: {
          labels: labels,
          datasets: [{
            label: 'Prestigio',
            data: rows.map(function(r) { return r.prestige || 0; }),
            borderColor: '#5bc8f5',
